...
- **Collapsible filters**: every filter's heading is a `.filter-head` button, not a label — it opens and closes the `.filter-body` under it, with the same `grid-template-rows: 1fr → 0fr` animation as the excluded-projects list. In a single column the two multiple-choice filters can take half a screen on their own, so closing what is not needed right now is what keeps the column short. **Closed does not mean off**: the selection lives in the hidden `<select>`s, which stay *outside* the collapsing body, so a closed filter keeps filtering. The state is kept in `localStorage` (`dashboardCollapsedFilters`) like the excluded projects — a preference about the page, not a filter of this visit — and it is reapplied on load behind a `.filters-no-anim` class removed after a synchronous reflow, so a closed filter is born closed instead of closing before one's eyes. The reflow replaces a `requestAnimationFrame`, which never arrives in a background tab (where the dashboard reloads on its own) and would leave the animation switched off for good. `#fi-search` lost its `<label for>` in the change, so the input carries its own `aria-label`.
...
- **Multiple-choice filters**: **Progetto** and **Sessione** both keep their selection in a hidden `<select multiple>` and show it as a list of checkboxes (`.check-list`), read back by `selectedProjects()` / `selectedSessions()`. No box ticked means all of them; ticking more than one puts them in OR (`filterByProject`, `filterBySession`). The two go through a single table, `MULTI_FILTERS`, which `clearFilter` and `applyChartFilter` address instead of naming either filter.
- **The sessions window** (`#session-picker`): the sessions are not picked in the filters column but in a modal opened by the last row of the Sessione block. There are hundreds of them against a handful of projects, and in the column the list would have buried every other filter. The window's markup sits **before** the page script, next to `#custom-tooltip`, because the script looks it up by id while loading. Every tick applies right away, so there is no confirm/cancel pair: `Fatto`, Esc and a click on the backdrop all just close.
...
- **The Sessione block looks like the Progetti one** (`#session-chosen`, `renderChosenSessions`): same `.check-list`, same "Tutte le sessioni" `.check-all` row on top (lit exactly when nothing is ticked, and the way to empty the selection), same `.check-row`s with a real ticked box — unticking removes that session, the same gesture one makes an inch further up for a project. They are the page's two multiple-choice filters and behave the same, so they must look the same; a dropdown button on one side and boxes on the other made that not read. The one piece the block has beyond the projects' is the last row, dressed like their "Esclusi sempre" heading, which opens the window: projects are a handful and all fit, sessions are hundreds. `.check-row` carries a `min-height` so both lists share one rhythm — the projects' rows are taller by the "exclude" command they hold, and two lists with a different step show as soon as they sit one under the other. It is rebuilt whole at every change: a handful of rows, against keeping count, "all" row and order aligned by hand.
...
- **Filtri richiudibili**: l'intestazione di ogni filtro è un bottone `.filter-head`, non un'etichetta — apre e chiude il `.filter-body` sotto di sé, con la stessa animazione `grid-template-rows: 1fr → 0fr` dell'elenco dei progetti esclusi. In una colonna sola i due filtri a scelta multipla possono prendersi mezzo schermo per conto loro, quindi chiudere ciò che adesso non serve è ciò che tiene corta la colonna. **Chiuso non vuol dire spento**: la selezione vive nei `<select>` nascosti, che restano *fuori* dal corpo che si chiude, quindi un filtro chiuso continua a filtrare. Lo stato sta in `localStorage` (`dashboardCollapsedFilters`) come i progetti esclusi — è una preferenza sulla pagina, non un filtro di questa visita — e si rimette al caricamento dietro una classe `.filters-no-anim` tolta dopo un reflow sincrono, così un filtro chiuso nasce chiuso invece di chiudersi davanti agli occhi. Il reflow sostituisce un `requestAnimationFrame`, che in una scheda in secondo piano (dove la dashboard si ricarica da sola) non arriva mai e lascerebbe l'animazione spenta per sempre. `#fi-search` ha perso la sua `<label for>` nel passaggio, quindi il campo porta un `aria-label` proprio.
...
- **Filtri a scelta multipla**: **Progetto** e **Sessione** conservano entrambi la selezione in un `<select multiple>` nascosto e la mostrano come lista di caselle (`.check-list`), riletta da `selectedProjects()` / `selectedSessions()`. Nessuna casella spuntata vuol dire tutte; spuntarne più d'una le mette in OR (`filterByProject`, `filterBySession`). I due passano da un'unica tabella, `MULTI_FILTERS`, a cui si rivolgono `clearFilter` e `applyChartFilter` invece di nominare l'uno o l'altro.
- **La finestra delle sessioni** (`#session-picker`): le sessioni non si scelgono nella colonna dei filtri ma in una finestra che apre l'ultima riga del blocco Sessione. Sono centinaia contro una manciata di progetti, e in colonna la lista avrebbe sepolto tutti gli altri filtri. Il markup della finestra sta **prima** dello script della pagina, accanto a `#custom-tooltip`, perché lo script la cerca per id già mentre si carica. Ogni spunta si applica subito, quindi non c'è nessuna coppia conferma/annulla: `Fatto`, Esc e un clic sul fondo scuro si limitano a chiudere.
...
- **Il blocco Sessione somiglia a quello Progetti** (`#session-chosen`, `renderChosenSessions`): stessa `.check-list`, stessa riga `.check-all` "Tutte le sessioni" in cima (accesa proprio quando non c'è niente di spuntato, ed è il modo di svuotare la selezione), stesse `.check-row` con una casella vera già spuntata — toglierla toglie quella sessione, lo stesso gesto che si fa un centimetro più su per un progetto. Sono i due filtri a scelta multipla della pagina e si comportano uguale, quindi devono anche assomigliarsi; un bottone-tendina da una parte e delle caselle dall'altra lo facevano sembrare falso. L'unico pezzo in più rispetto ai progetti è l'ultima riga, vestita come la loro intestazione "Esclusi sempre", che apre la finestra: i progetti sono una manciata e ci stanno tutti, le sessioni sono centinaia. `.check-row` porta un `min-height` perché le due liste abbiano lo stesso passo — quelle dei progetti sono più alte per il comando "escludi" che contengono, e due liste con il passo diverso si notano appena stanno una sotto l'altra. Si rifà per intero ad ogni cambio: una manciata di righe, contro il tenere allineati a mano conteggio, riga "tutte" e ordine.
