...
- **Preference combos (`.pref-combo`)**: one single object, generated by walking the registries in `i18n.py` and wired by a single loop in `I18N_APPLY`, which reads `data-pref` on each combo and asks `window.__prefs` for the active value. Adding a third preference means one line in the `PREFS` table and one entry in `render_header`.
  ...
  - **Currency**: symbol and ISO code, to the left of the language, **on the dashboard only**: the price list and the guide report the official tariff, which is in dollars, and there the combo would have nothing to change.
...
- **Combo delle preferenze (`.pref-combo`)**: un oggetto solo, generato scorrendo i registri di `i18n.py` e cablato da un unico ciclo in `I18N_APPLY`, che legge `data-pref` su ogni combo e chiede a `window.__prefs` qual è il valore attivo. Aggiungere una terza preferenza significa una riga nella tabella `PREFS` e una voce in `render_header`.
  ...
  - **Valuta**: simbolo e codice ISO, a sinistra della lingua, **solo sulla dashboard**: tariffario e guida riportano il listino ufficiale, che è in dollari, e lì la combo non avrebbe niente da cambiare.
